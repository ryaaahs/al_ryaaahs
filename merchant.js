...
function al_logger(type, data) 
{
	...
	switch(type)
	{
		case "DEBUG": if (DEBUG) console.debug(`${timestamp.toISOString()} [DEBUG] ${data}`);
		break;
		case "ERROR": console.error(`${timestamp.toISOString()} [ERROR] ${data}`)
		break;
		case "INFO": console.info(`${timestamp.toISOString()} [INFO] ${data}`)
		break;
	}
} 
...
setInterval(() => {
	...
	if ((character.mp <= character.mp_cost || character.mp <= character.max_mp - mp_usage) && !is_on_cooldown("use_mp")) {
		use_skill("use_mp")
	}
	...
	if (farmers.includes(farmer_name))
	{
		for (farm_character of character_list)
		{
			if(farm_character.name == farmer_name)
			{
				...
				const server_name = server.includes("ASIA") ? "ASIA" : server.substring(0, 2); 
				// Move to their server
				change_server(server_name, server.substring(server_name == "ASIA" ? 4 : 2,  server.length));
			}
		}
...
/**
* Parses through the inventory and either upgrades/compounds based on items_to_upgrade list
*
*/ 
async function upgrade_inventory()
{
	...
	{	
		...
		if (character.items[i] && (character.items[i]?.level || character.items[i].level == 0)) 
		{
			...
			if (items_to_upgrade[character.items[i].name])
			{
				if (G.items[character.items[i].name].upgrade) 
				{
					...
					while (character.items[i] && character.items[i].level < items_to_upgrade[character.items[i].name].target_level)
					{
						// Check the grade of the item for the scroll
						scroll_index = locate_item(`scroll${item_grade(character.items[i])}`); 
						if (scroll_index == -1) break; 	
						
						al_logger("INFO", `Upgrading item "${item_name}" at index ${i}`) 
						try 
						{	
							if (!is_on_cooldown("massproduction")) await use_skill("massproduction");
				...
				else if (G.items[character.items[i].name].compound)
				{
					...
					// Check the grade of the item for the scroll
					scroll_index = locate_item(`cscroll${item_grade(character.items[i])}`); 
					
					// Interate through all the inventory items to get the other two items for compound
					for (let j = 0; j < inventory_length; j++)
					{
						...
						{
							if (character.items[j].name == character.items[i].name && character.items[j].level == character.items[i].level)
							{
								item_group.push(j);
							}
						}
						...
						if (item_group.length == 3)
						{
							...
							{
								if (!is_on_cooldown("massproduction")) await use_skill("massproduction");
								await compound(item_group[0], item_group[1], item_group[2], scroll_index);
							...
							{
								al_logger("ERROR", `Failed to compound item: ${e}: ${e.response}`);
								al_logger("DEBUG", `Item Name: ${item_name}`);  
								al_logger("DEBUG", `Index: ${i}`);
								al_logger("DEBUG", `Scroll Index: ${scroll_index}`);
								break;
